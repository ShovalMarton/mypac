...
#' Expected to be one of the follow: CosK(x) EpaK(x) Epa2K(x) gaussK(x),from locpol package.
...
#'   
#' @export
CIF = function(T1,T2,C,R,S_LT,BW_method,Boot = 100,file.name){
  ...
  if(Boot>0){
    ...
    for(iter in 1:Boot){
      ...
      new_fun <- CIF_new_kernel(main_data = main_data_boot,S_LT = S_LT,digits = 0,LT = 40,BW_method = BW_method)
